...
%dela = SimOut.Vehicle.SurfAct.Position.Data(:,2);
dela = table2array(timeseries2timetable(del_a));
delr = SimOut.Vehicle.SurfAct.Position.Data(:,5);
%k = table2array(timeseries2timetable(left_ail));

d_dele = SimOut.Vehicle.SurfAct.Rate.Data(:,3);
%d_dela = SimOut.Vehicle.SurfAct.Rate.Data(:,1);
d_dela = gradient(dela, time);
...
plot(time, [table2array(timeseries2timetable(left_ail)) table2array(timeseries2timetable(right_ail)) dela d_dela]*180/pi)
ylabel('aileron','fontsize',15)
legend({'$Left Aileron$ [deg]','$Right Aileron$ [deg]','$\delta Ail$ [deg]','$\dot{\delta Ail}$ [deg/sec]'}, 'fontsize', 12,'Interpreter','latex')
axis( [min(time) max(time) -35 35])
grid on;
zoom on;
